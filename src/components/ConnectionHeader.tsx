  ...
  return (
    <header className="bg-white dark:bg-slate-800 border-b border-slate-200 dark:border-slate-700 px-6 shadow-sm shrink-0 z-10">
      ...
      {connectionMode !== "none" && pathSegments.length > 0 && (
        <div className="h-6 flex items-center gap-1 pb-1 overflow-x-auto">
          <div className="flex items-center gap-0.5 text-[10px] font-mono text-slate-400 dark:text-slate-500 whitespace-nowrap">
            {pathSegments.map((seg, i) => (
              <span key={i} className="flex items-center gap-0.5">
                {i > 0 && <ChevronRight className="w-2.5 h-2.5 text-slate-500 shrink-0" />}
                <button
                  onClick={() => handleBreadcrumbClick(i)}
                  className="hover:text-indigo-600 dark:hover:text-indigo-400 hover:bg-slate-100 dark:hover:bg-slate-700/50 px-1 py-0.5 rounded transition-colors cursor-pointer"
                >
                  {seg}
                </button>
              </span>
          ...
          <button
            ...
            className="ml-0.5 p-0.5 rounded text-slate-500 hover:text-indigo-600 dark:hover:text-indigo-400 hover:bg-slate-100 dark:hover:bg-slate-700/50 transition-colors cursor-pointer shrink-0"
            title="Copiar ruta"
          >
